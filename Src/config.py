...
class Config(object):
    ...
    def get_domain(self, tag, args, path, debug=True):
        if tag[:9] == 'Gridworld':
            ...
            obj = Utils.dynamic_load(path, tag, load_class=True)
            env = obj(smin=self.smin,smax=self.smax,n_items=args.n_actions,max_steps=args.max_steps,commonOrderCosts=args.commonOrderCosts,mappingType=self.mapping, demand_dist=args.demand_dist, demand_correlation=args.demand_correlation)
            return env, False, env.action_space.dtype == np.float32
        elif tag[:7] == 'JobShop':
            obj = Utils.dynamic_load(path, tag, load_class=True)
